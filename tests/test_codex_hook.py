"""Tests for the OpenAI Codex CLI PreToolUse hook script."""
...
HOOK_SCRIPT = str(Path(__file__).parent.parent / "cross" / "patches" / "codex_hook.py")
...
class TestCodexHookSkip:
    def test_skips_when_openai_base_url_is_localhost(self):
        exit_code, _, _ = _run_hook(env_override={"OPENAI_BASE_URL": "http://localhost:2767"})
        assert exit_code == 0

    def test_skips_when_openai_base_url_is_127(self):
        exit_code, _, _ = _run_hook(env_override={"OPENAI_BASE_URL": "http://127.0.0.1:2767"})
        assert exit_code == 0
    ...
    def test_does_not_skip_when_no_proxy_env(self):
        """Without proxy env vars, the hook should proceed (and fail open if daemon unreachable)."""
        exit_code, _, _ = _run_hook(
            tool_name="Bash",
            tool_input={"command": "ls"},
        )
        assert exit_code == 0  # Fails open since daemon isn't running


class TestCodexHookFailOpen:
...
class TestCodexHookGateIntegration:
    ...
    def test_sends_correct_payload(self, gate_server):
        ...
        assert payload["session_id"] == "test-codex-session"
